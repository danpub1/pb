...
* `max-pages`: Maximum number of pages per PDF file. [0]
...
* `binding`: The book's binding location, one of `side`, `top`, `none`.  Controls if margins are alternated by even/odd pages. [none]
...
* `pack`: Include this when packing. [true]
...
Book-Level Options
------------------
* `assemble`: Assembles pages from PDFs for printing. []
...
------------------
* `output-file`: . [out.pdf]
* `noprocess`: Do not layout, resize, or render the page. [false]
